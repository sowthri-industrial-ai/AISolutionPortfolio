...
- Zero-RBAC OIDC verification requires `azure/login`'s `allow-no-subscriptions: true` **and** omission of `subscription-id` — the action's default path assumes ≥1 subscription visible to the principal and errors "No subscriptions found" otherwise (the SP has zero roles by the 5b posture). The zero-roles proof is therefore tenant-level (`az account show` returns the SP identity + tenant, no subscription). *Mitigation:* documented here; Phases 5c/5d grant RG-scoped roles so they pass `subscription-id` and do **not** set `allow-no-subscriptions` — this interaction is unique to the deliberately zero-RBAC 5b trust-only workflow.
